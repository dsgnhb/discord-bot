exports.run = async (client, message, level) => {
  ...
  if(possibleKeys.length > 0) {
      ...
      const random = reactions[name].output[Math.floor(Math.random() * reactions[name].output.length)].toUpperCase();
      message.guild.members.get(client.user.id).setNickname(name);
      message.channel.send(name + ': ' + random)
      client.log('log', `${name} reacted on ${message.author.username}'s (${message.author.id}) message with answer "${random}"`, 'MarianeReacts')
