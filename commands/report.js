...
exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-lets
    ...
    /*request.get({
       ...
       json: true
        ...
        let video = body.items[0].id.videoId;
        console.log(video);
       // message.channel.send(video);
        });*/
    await client.wait(10000)
    let messages = ["Puuh! Die Gomme-Mods sind am schwitzen und scheinen hier etwas überfordert.", "Leider mussten alle Gomme-Mods schon ins Bett.", "Alle Gomme-Mods sind gerade am kacken. Versuchs in "+errorcode + " Stunden nochmal. loool", "Error 404, welcher Vollidiot hat den TS heruntergefahren?"]
