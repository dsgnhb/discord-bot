...
module.exports = client => {
  // Twitter User ID from gettwitterid.com
  const user = '826091131062595586'

  const twitter = new Twit(client.config.tokens.twitter)
  const twStream = twitter.stream('statuses/filter', { follow: user })
  ...
  twStream.on('connect', () => client.log('Log', 'Connecting to Twitter API', 'Twitter'))
  twStream.on('connected', () => client.log('Log', 'Connected to Twitter API', 'Twitter'))
  twStream.on('disconnect', disconnectMessage => client.log('Log', `Disconnected from Twitter API: ${disconnectMessage}`, 'Twitter'))
  twStream.on('reconnect', () => client.log('Log', 'Reconnecting to Twitter API', 'Twitter'))
  twStream.on('warning', warning => client.log('Log', `Warning from Twitter API: ${warning}`, 'Twitter'))
}
